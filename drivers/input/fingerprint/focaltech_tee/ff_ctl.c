...
#define PROC_FP_INFO "driver/fp_info"
char mtk_fp_info[10] = {0};

/* Touchpanel FW ver */
static int subsys_fp_read(struct seq_file *m, void *v)
{
    pr_err("subsys_fp_read %s\n",mtk_fp_info);
   seq_printf(m, "%s\n",mtk_fp_info);
   return 0;
};

static int proc_fp_info_open(struct inode *inode, struct file *file)
{
    return single_open(file, subsys_fp_read, NULL);
};

static  struct file_operations fp_proc_fops = {
    .owner = THIS_MODULE,
    .open  = proc_fp_info_open,
    .read  = seq_read,
};
//end
...
//Start fw ver
       proc_create(PROC_FP_INFO, 0, NULL, &fp_proc_fops);
//end
...
//    init_flag=1;
    ...
    if (ff_ctl_init_driver() == 0) {
        ...
        snprintf(mtk_fp_info,sizeof(mtk_fp_info),"%s","ft9371");
	    /*if (ff_chip_init() == 0x9372) {
