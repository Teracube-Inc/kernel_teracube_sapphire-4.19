...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
	...
	sprintf(mtk_tp_info, "%s", "chipone-ICNL9911C");
...
{
    return cts_dev->rtdata.proximity_enable;
}
...
int cts_enter_proximity_mode(struct cts_device *cts_dev)
{
    ...
    ret = cts_send_command(cts_dev, CTS_CMD_ENABLE_PROXIMITY);
    if (ret) {
        cts_err("Enable proximity failed, %d", ret);
        return ret;
    }

    cts_dev->rtdata.proximity_enable = true;
...
int cts_exit_proximity_mode(struct cts_device *cts_dev)
{
    ...
    ret = cts_send_command(cts_dev, CTS_CMD_DISABLE_PROXIMITY);
    if (ret) {
        cts_err("Disable proximity failed, %d", ret);
        return ret;
    }

    cts_dev->rtdata.proximity_enable = false;
    ...
    if (retries < 0) {
        ...
        if (firmware) {
            ...
            cts_err("Request default firmware failed %d(%s), "
				"please update manually!!", ret, cts_strerror(ret));
...
int cts_enter_glove_mode(const struct cts_device *cts_dev)
{
    ...
    ret = cts_send_command(cts_dev, CTS_CMD_ENABLE_GLOVE);
    if (ret) {
...
int cts_exit_glove_mode(const struct cts_device *cts_dev)
{
    ...
    ret = cts_send_command(cts_dev, CTS_CMD_DISABLE_GLOVE);
    if (ret) {
