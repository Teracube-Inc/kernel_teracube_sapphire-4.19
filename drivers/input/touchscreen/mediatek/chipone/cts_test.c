        ...
        struct cts_test_param *param)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        r = cts_enable_get_rawdata(cts_dev);
        if (r) {
            cts_err("Enable get ts data failed %d(%s)", r, cts_strerror(r));
            continue;
        }
        mdelay(1);
        r = cts_fw_reg_readb(cts_dev, 0x12, &val);
        if (r) {
            cts_err("Read enable get ts data failed %d(%s)", r, cts_strerror(r));
            continue;
        }
        if (val != 0) {
            break;
    ...
    for (frame = 0; frame < priv_param->frames; frame++) {
        ...
        r = cts_send_command(cts_dev, CTS_CMD_QUIT_GESTURE_MONITOR);
        if (r) {
            cts_err("send quit gesture monitor failed %d(%s)", r, cts_strerror(r));
            // Ignore this error
        }
    ...
    if (driver_validate_data) {
        ...
        if (dump_test_data_to_user && param->test_data_buf_size >=
			(*param->test_data_wr_size + tsdata_frame_size)) {
...
disable_get_tsdata:
    for (i = 0; i < 5; i++) {
        int r = cts_disable_get_rawdata(cts_dev);
        if (r) {
            cts_err("Disable get rawdata failed %d(%s)", r, cts_strerror(r));
            continue;
        } else {
            break;
        }
