...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.pre = {
		...
		.linelength = 3448, //0xD78				//record different mode's linelength
...
extern bool is_factory_mode(void);// zwb modifed for CTS Test
static kal_uint32 set_test_pattern_mode_bl(kal_bool enable)
{
	printk("enable: %d\n", enable);
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ_BY_SCENARIO:
		...
		case SENSOR_FEATURE_SET_TEST_PATTERN:
           if (is_factory_mode()) //zxs 20220526
           {
				printk("is_factory_mode:%d\n", __LINE__);
				set_test_pattern_mode((BOOL)(*feature_data));

            }
			else{
			    printk("is not factory_mode:%d\n", __LINE__);
                set_test_pattern_mode_bl((BOOL)*feature_data);
			}
			break;
