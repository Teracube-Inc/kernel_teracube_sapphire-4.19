...
#if defined(BUILD_LK)
	printf("icnl9911c buffer = 0x%x ,  0x%x ,id = 0x%x \n",buffer[0],buffer[1],id);
#else
	printk("icnl9911c buffer = 0x%x ,  0x%x ,id = 0x%x \n",buffer[0],buffer[1],id);
#endif

	return (id == 0x9911) ? 1 : 0;
 }                                     
