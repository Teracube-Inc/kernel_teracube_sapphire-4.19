...
#define BAT_LIMIT_CURRENT_1A    (1000000)
...
int eta6937_get_chip_status(struct charger_device *chg_dev, bool *done)
{
	unsigned int ret = 0;
	unsigned char val = 0;
	static unsigned int report_full = 0;
...
#if 0//defined(YK676_V60_CUSTOMER_TRX_S607_HDPLUS)
extern bool chr_current_limi;
#endif

/*
（0℃~15℃≤1A电流充电，15℃~45℃恢复正常电流充电，45℃后充电电流限流1A，充电电池限压4.15V）
*/
...
static int eta6937_set_current(struct charger_device *chg_dev, u32 current_value)
{
	...
	pr_debug("liml_bat eta6937_set_current=%u\n",current_value);
#if 0//defined(defined(YK676_V60_CUSTOMER_TRX_S607_HDPLUS)
	if(chr_current_limi)
		return status;
#endif
	...
	if(g_battery_temp > 45||g_battery_temp<15){
		if(current_value > BAT_LIMIT_CURRENT_1A)
		   current_value = BAT_LIMIT_CURRENT_1A;
	}
	...
	if (current_value <= 35000) {
		...
		pr_debug("eta6937 charging_set_current  set_chr_current=%u\n", set_chr_current);
		
		register_value = charging_parameter_to_value(eta6937_CS_VTH, array_size, set_chr_current);
		pr_debug("eta6937 charging_set_current  register_value=%u\n", register_value);
...
static int eta6937_set_input_current(struct charger_device *chg_dev, u32 current_value)
{
    ...
    eta6937_get_chip_status(chg_dev,&done);
	pr_debug("liml_bat eta6937_set_input_current=%u,g_battery_temp=%d,charge_done=%d\n",current_value, g_battery_temp,done);
	if(done){
		pr_debug("liml_bat eta6937_set_input_current reset ic\n");

		eta6937_enable_charging(chg_dev, 0);
		msleep(5);
		eta6937_enable_charging(chg_dev, 1);
		// msleep(10);
	}
	eta6937_set_current(chg_dev, current_value);
	...
	printk("liml_bat after eta6937_set_input_current=%u,g_battery_temp=%d g_input_current=%u\n",current_value, g_battery_temp,g_input_current);
...
static int eta6937_set_cv_voltage(struct charger_device *chg_dev, u32 cv)
{
	...
	u32 array_size;
	u32 set_cv_voltage;
	if(g_battery_temp > 45){
		cv = 4150000;
        printk("liml_bat eta6937_set_cv_voltage=%d,g_battery_temp=%d\n",cv, g_battery_temp);
	}
	...
	pr_debug("eta6937 eta6937_set_cv_voltage  register_value=%u,set_cv_voltage=%u\n", register_value,set_cv_voltage);
...
void eta6937_set_otg_enable(void)
{
    ...
    while(retry_times < 20){
		...
		val = eta6937_get_otg_status();
        printk("eta6937 otg state =%d", val);

	    eta6937_set_opa_mode(1);
	    retry_times++;
	    eta6937_read_byte(1, &eta6937_reg[1]);
	    printk("eta6937[1]=0x%x retry_times=%d", eta6937_reg[1],retry_times);

	    if(((eta6937_reg[1])&1) == 1)
	    break;
	    msleep(200);
    }
...
#if defined(HIGH_BATTERY_VOLTAGE_SUPPORT)
	#if defined(HIGH_BATTERY_4_4_V)
		eta6937_reg_config_interface(0x06, 0x7C); //7F
		eta6937_reg_config_interface(0x02, 0xBA); //B6
	#else
